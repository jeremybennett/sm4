...
{
  return ((uint32_t)b[4 * n + 3] << 24) | ((uint32_t)b[4 * n + 2] << 16)
         | ((uint32_t)b[4 * n + 1] << 8) | ((uint32_t)b[4 * n]);
}
...
static void
sm4_key_schedule (uint8_t key[], uint32_t rk[])
{
  uint32_t t, x, k[36];
  int i;
...
void
sm4_encrypt (uint8_t key[], unsigned char plaintext[],
...
void
sm4_decrypt (uint8_t key[], unsigned char ciphertext[],
