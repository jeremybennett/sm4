  ...
  if (tc.encrypt)
    {
      ...
      for (i = 0; i != tc.iterations; ++i)
        {
          sm4_encrypt (tc.key, (unsigned char *)input, (unsigned char *)output);
  ...
  else
    {
      ...
      for (i = 0; i != tc.iterations; ++i)
        {
          sm4_decrypt (tc.key, (unsigned char *)input, (unsigned char *)output);
          memcpy (input, output, SM4_BLOCK_SIZE);
        }
    }

  /* The orignal code has:

       debug = 1;

     which leads to an undefined variable error. I presume it was intended to
     use the #define to force debugging on and off.
  */
...
#define DEBUG 0
  ...
  return memcmp ((unsigned char *)output, (unsigned char *)tc.expected,
                 SM4_BLOCK_SIZE);
}
...
main (int argc, char **argv)
{
  ...
  test_case gbt32907t1
      = { (unsigned char *)gbt32907k1, (unsigned char *)gbt32907m1,
          (unsigned char *)gbt32907e1, 1, true };
  tests[0] = gbt32907t1;
  ...
  test_case gbt32907t2
      = { (unsigned char *)gbt32907k1, (unsigned char *)gbt32907m1,
          (unsigned char *)gbt32907e2, 1000000, true };
  tests[1] = gbt32907t2;
  ...
  test_case gbt32907t3
      = { (unsigned char *)gbt32907k1, (unsigned char *)gbt32907e1,
          (unsigned char *)gbt32907m1, 1, false };
  tests[2] = gbt32907t3;
  ...
  test_case gbt32907t4
      = { (unsigned char *)gbt32907k1, (unsigned char *)gbt32907e2,
          (unsigned char *)gbt32907m1, 1000000, false };
  tests[3] = gbt32907t4;
  ...
  test_case newexample1
      = { (unsigned char *)newexamplek1, (unsigned char *)newexamplem1,
          (unsigned char *)newexamplee1, 1, true };
  tests[4] = newexample1;

  test_case newexample2
      = { (unsigned char *)newexamplek1, (unsigned char *)newexamplee1,
          (unsigned char *)newexamplem1, 1, false };
  tests[5] = newexample2;
  ...
  static const unsigned int newexamplee2[SM4_BLOCK_SIZE]
      = { 0x379a96d0, 0xa6a5a506, 0x0fb460c7, 0x5d1879ed };
  test_case newexample3
      = { (unsigned char *)newexamplek1, (unsigned char *)newexamplem1,
          (unsigned char *)newexamplee2, 1000000, true };
  tests[6] = newexample3;
